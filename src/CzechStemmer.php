...
class CzechStemmer
{
	...
	public function stemmLight($word)
	{
	...
	public function stemmAgressive($word)
	{
		$word = mb_strtolower($word);
		$word = $this->removeCase($word);
		$word = $this->removePossessives($word);
	...
	private function removeCase($word)
	{
	...
	private function palatalise($word)
	{
		if (
			mb_substr($word, -2) === 'ci'
			|| mb_substr($word, -2) === 'ce'
	...
	private function removePossessives($word)
	{
		if (mb_strlen($word) > 5) {
	...
	private function removeDerivational($word)
	{
		$length = mb_strlen($word);
	...
	private function removeAugmentative($word)
	{
		$length = mb_strlen($word);
	...
	private function removeDiminutive($word)
	{
		$length = mb_strlen($word);
	...
	private function removeComparative($word)
	{
